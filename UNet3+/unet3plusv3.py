...
def UNet3Plus(input_shape):
    ...
    hd1 = conv_block(hd1, 320)  # shape=(None, 320, 320, 320)

    # last layer does not have batchnorm and relu
    #d = conv_block(d1, output_shape, n=n, is_bn=False, is_relu=False)

    d = keras.layers.UpSampling2D(size = (2,2), interpolation= 'bilinear')(hd1) #(None,448,448,320)
    d = keras.layers.Conv2D(3, 3, padding='same')(d) #(None, 448,448,3)
    ...
    return keras.Model(inputs=inputs, outputs=output, name='UNet_3Plus')
    ...
    # #model=keras.Model(inputs=inputs,outputs=d)
...
if __name__ == '__main__':
    model=UNet3Plus(input_shape=(224,224,4))
    model.summary()
